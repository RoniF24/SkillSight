"""codes/models/OnePass/data_onepass.py

Data utilities for the OnePass model.

Responsibilities:
- Read dataset splits from JSONL (`train.jsonl`, `val.jsonl`, `test.jsonl`).
- Load the closed skill list (`skills_used.txt`) and map skills -> indices.
- Convert each example's `skills` dict into a per-skill 3-class label vector:
    0 = NONE, 1 = IMPLICIT (0.5), 2 = EXPLICIT (1.0)
- Provide a `torch.utils.data.Dataset` and ready-to-use `DataLoader`s with padding.

Training logic, model definition, and evaluation live in `train_onepass.py` and `eval_onepass_topk.py`.
"""
