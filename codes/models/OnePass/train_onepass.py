...
def find_repo_root() -> Path:
    ...
    for p in [start.parent] + list(start.parents):
        if (p / "codes").exists() and ((p / "data").exists() or (p / "DATA").exists()):  # ✅ robust
            return p
...
def main() -> None:
    ...
    # ✅ changed: output folder name outside codes (like pairwise)
    ap.add_argument("--models_dir", type=str, default="trained onepass")  # ✅ requested name
    ...
    # ✅ robust in case it's DATA instead of data
    splits_dir = resolve_data_like_path(repo, args.splits_dir)
    if not splits_dir.exists():
        raise FileNotFoundError(f"splits_dir not found: {splits_dir}")

    # ✅ changed: remove 'one_pass' subfolder to match exactly "trained onepass/<run_name>"
