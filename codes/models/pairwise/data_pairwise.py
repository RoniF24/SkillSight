"""codes/models/pairwise/data_pairwise.py

Data utilities for the Pairwise model.

Pairwise framing:
- Each training example is (skill, job_description) -> label
- Labels are 3-way:
        0 = NONE (skill not present)
        1 = IMPLICIT (skill present indirectly; score 0.5 in JSONL)
        2 = EXPLICIT (skill mentioned verbatim; score 1.0 in JSONL)

This module:
- Reads split JSONL files.
- Loads the closed skill list (`skills_used.txt`).
- Converts each JSONL item into a list of per-skill `PairExample`s.
    For training we sample negatives to keep the dataset size manageable.
    For evaluation you may optionally generate the full cartesian product (eval_full=True).

Training logic lives in `train_pairwise.py`. JD-level evaluation/top-k selection lives in `eval_pairwise_topk.py`.
"""
...
def build_pair_examples(
    ...
    for idx_item, it in enumerate(items):
        ...
        # Sample negatives so the dataset doesn't explode (skills list can be large).
        # We scale negatives with the number of positives and enforce a small minimum.
