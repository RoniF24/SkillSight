...
# ✅ NEW: where pairwise models are saved now (outside codes)
...
def jd_level_metrics(
    ...
    """
    🔥 NEW RULE: a prediction counts as correct ONLY if:
      - skill name matches AND
      - type matches exactly (0.5 vs 1.0)

    So precision/recall/f1 are now "typed" (skill+type).
    """
...
def main():
    ...
    # ✅ updated agg keys to match new jd_level_metrics output
    agg = {
        "precision_at_k": 0.0,
        "recall_at_k": 0.0,
        "f1_at_k": 0.0,
