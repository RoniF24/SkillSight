"""codes/models/pairwise/train_pairwise.py

Training entry point for the Pairwise classifier.

Pairwise framing:
- Each example is (skill, job_description) -> label in {0,1,2}
    0 = NONE, 1 = IMPLICIT, 2 = EXPLICIT

This script:
- Loads split JSONL data and the closed skill list from `data/splits_v1/` (by default).
- Builds pair examples via `data_pairwise.build_pair_examples()` (negative sampling for training).
- Trains a 3-class HuggingFace `AutoModelForSequenceClassification` with `transformers.Trainer`.
- Optionally uses class-weighted cross-entropy to reduce collapse to the majority class.
- Writes `run_config.json`, training history, and a final model under `--output_dir`.

JD-level top-k evaluation/aggregation is implemented in `eval_pairwise_topk.py`.
"""
...
def main() -> None:
    ...
    # Train once; downstream evaluation scripts load from `out_dir`.
